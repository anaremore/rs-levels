...
| Sierra Chart | `plugins/sierra-chart/varis-zones-sierra.cpp` | `GET /stats/:symbol/rows` | Build ACSIL DLL, add study, verify Auto symbol detection on ES/MES and NQ/MNQ charts, `API RI` status from captured or DD-derived RI, manual fallback before RI is available, VWAP, and RI bands. |
