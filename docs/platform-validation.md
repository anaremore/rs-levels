...
| Sierra Chart | `plugins/sierra-chart/varis-zones-sierra.cpp` | `GET /stats/:symbol/rows` | Build ACSIL DLL, add study, verify the status build tag changed, `Follow chart symbol` is on, Auto symbol detection works on ES/MES and NQ/MNQ charts, `API RI` status comes from captured or DD-derived RI, manual fallback works before RI is available, and VWAP/RI bands render. |
