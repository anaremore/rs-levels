...
The direct-polling adapters consume the Sierra-compatible `name,price,red,green,blue,kind` text feed. Newer adapters use the optional sixth `kind` column for category-aware colors. Sierra Chart, NinjaTrader, and Quantower also offset labels away from the level line and fill matched zone top/bottom pairs when their chart drawing APIs support it. Bookmap maps kinds into distinct value-line color slots.
...
- the implemented `/tradingview` local-service endpoint and extension button to copy a compact all-symbol level payload
- the implemented `/tradingview/:symbol?format=json` export for selected-symbol users and tooling
