...
The direct-polling adapters consume read-only symbol level feeds. Sierra Chart prefers the normal JSON `GET /levels/:symbol` response and keeps a row-feed fallback. NinjaTrader, Quantower, and Bookmap consume the generic display row `name,price,red,green,blue,kind` text feed at `GET /levels/:symbol?format=rows`. Adapters use the level `kind` for category-aware colors, including user-added yellow, red, and CAT manual lines. Sierra Chart, NinjaTrader, and Quantower also offset labels away from the level line and fill matched zone top/bottom pairs when their chart drawing APIs support it. Bookmap maps kinds into distinct value-line color slots.

Direct adapters also consume `GET /stats/:symbol` or `GET /stats/:symbol?format=rows` for chart-corner display context such as `DD`, `Res`, `MRes`, `WRes`, and `Map BLD`. These values are display-only context from RocketScooter and are not interpreted as trading signals by this repository.
