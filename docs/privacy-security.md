...
- The latest sanitized TradingView snapshot is stored in browser-managed extension `storage.session`, not page `localStorage`. Raw capture bodies are not stored there. The record is cleared by browser restart or extension update.
