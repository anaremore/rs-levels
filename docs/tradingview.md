...
Optional manual RocketScooter lines are not generated by the TradingView indicator or local API. To include overnight HP/MHP, yellow lines, red lines, or CAT lines, add or keep them visible on the relevant RocketScooter ES/MES or NQ/MNQ futures chart before clicking `Copy TradingView`. Copy a fresh payload after changing those lines.
