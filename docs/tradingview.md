...
- `Label layout`: `Rail` (default) docks high-contrast dark chips near the chart's visible right boundary, vertically separates only nearby collisions, and uses subtle dotted leaders for displaced labels. `On line` uses lightweight white text just off each price line and spreads crowded labels horizontally. `Hidden` removes labels while preserving lines and fills.
