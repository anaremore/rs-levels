...
New extractors should emit `zone-bull` or `zone-bear` for every zone. Generic `zone` remains schema-compatible for older or ambiguous captures, but display settings should not expose a separate generic Zones toggle; treat it as `Other levels` unless a platform needs a backward-compatible neutral fallback. When RocketScooter exposes zone rectangle geometry separately from the text label, capture both and use the exact rectangle top/bottom as the zone boundaries; a generic `Bull Zone` or `Bear Zone` label inside that rectangle should be consumed rather than exported as an extra horizontal level. DD-bounded reconstruction is only a fallback for older captures that expose a generic zone label but no rectangle geometry. When a platform can draw filled regions, matching top/bottom zone rows such as `BZT1`/`BZB1`, `BrZT1`/`BrZB1`, `Bull Zone Top`/`Bull Zone Bottom`, or `Bear Zone Top`/`Bear Zone Bottom` should be filled with a low-opacity version of the matching bull or bear zone color. Display adapters should canonicalize legacy or malformed zone text to the public labels `Bull Zone` and `Bear Zone`; a bear zone label must never render as `Bea Zone`.
...
Extractors and plugins should preserve RocketScooter's automatic overnight names as `Dyn MHP` and `Dyn HP`, while continuing to accept legacy manual `OVNMHP` and `OVNHP` rows. Yellow, red, and CAT lines remain source-owned manual levels. Adapters must not fabricate any level that is absent from the capture.
