...
assert.equal(body.chartLines.length, 7);
...
assert.equal(body.levels.filter((level) => level.symbol === 'MES' && level.name === 'Yellow Line' && level.kind === 'yellow-line').length, 2);
assert.equal(body.levels.filter((level) => level.symbol === 'MES' && level.name === 'Red Line' && level.kind === 'red-line').length, 2);
