...
assert.match(popup, /setPill\('PAUSED'/);
...
assert.match(popupHtml, /id="capture-info"/);
assert.match(popupHtml, /capture-disclosure/);
assert.match(popupHtml, /role="tooltip"/);
assert.match(popupHtml, /aria-label="About RocketScooter capture"/);
...
assert.match(popupCss, /\.capture-info:focus-within/);
